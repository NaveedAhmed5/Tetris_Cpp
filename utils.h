...
const char title[] = "Tetris Mint Edition";
...
bool anamoly(){
    for (int i=0;i<4;i++)
        if (point_1[i][0]<0 || point_1[i][0]>=N || point_1[i][1]>=M)	/*This condition prevents the bolock from falling below the 
        															allocated frame*/
            return 0;
        else if (gameGrid[point_1[i][1]][point_1[i][0]])	/*This condition prevents a block from overlapping another block*/
            return 0;
    return 1;
};
